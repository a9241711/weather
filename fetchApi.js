class getApi{
    ...
    async getFetch(keyword){
        ...
        try{
            let getData= await fetch(`https://opendata.cwb.gov.tw/api/v1/rest/datastore/F-C0032-001?Authorization=${apiKey}&limit=1&locationName=${keyword}`)
            let data= await getData.json();
            return data
        }
        catch(mess){
            throw Error("err",mess)
        }
    }
}
